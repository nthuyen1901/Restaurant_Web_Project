...
class LoginView(AuthLoginView):
    ...
    def get_success_url(self):
        ...
        if self.request.user.is_staff:
            return reverse_lazy('admin:index')  # Redirige vers le panneau d’administration de Django.
        return reverse_lazy('Home')  # Redirige vers la page d’accueil si l’utilisateur n’est pas administrateur.

def LogoutView(request):
    logout(request)
    messages.success(request, 'You have been logged out successfully.')
    return redirect('Home')  # Rediriger vers une page après la déconnexion, par exemple la page d’accueil.
...
def BookTableView(request):
    # Transmettre la clé API au template.
    ...
    if request.method == 'POST':
        ...
        if name != '' and len(phone_number) == 10 and email != '' and total_person != '0' and booking_data != '':
            # Enregistrer les données de réservation dans la base de données.
            ...
            recipient_list = [email]  # L’e-mail de l’utilisateur
            ...
            # Rediriger ou afficher une page de retour avec un message de succès.
            return render(request, 'feedback.html', {'success': 'Booking request submitted successfully! Please check your confirmation email.'})

    # Afficher le template book_table.html et lui transmettre la clé API.
